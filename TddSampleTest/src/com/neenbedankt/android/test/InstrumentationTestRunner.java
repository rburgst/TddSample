...
public class InstrumentationTestRunner extends android.test.InstrumentationTestRunner {
    ...
    public void onStart() {
        try {
        	final File PRIVATE_DIR = getTargetContext().getFilesDir();
        	final File EXTERNAL_DIR = new File("/mnt/sdcard/");
            startJUnitOutput(new FileWriter(new File(EXTERNAL_DIR, JUNIT_XML_FILE)));
        } catch (IOException e) {
    ...
    void recordTestStart(Bundle results) {
    	System.out.println(results.getString(REPORT_KEY_NAME_TEST) + " started.");
    ...
    void recordTestResult(int resultCode, Bundle results) throws IOException {
        ...
        if (resultCode != REPORT_VALUE_RESULT_OK) {
            ...
            mTestSuiteSerializer.attribute(null, "time", String.format(Locale.US, "%.3f", time));
        }
